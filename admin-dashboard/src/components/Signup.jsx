...
const Signup = () => {
    ...
    // Extract the admin from the URL query parameters when redirected after Google sign-in
    useEffect(() => {
        const params = new URLSearchParams(location.search);
        const admin = params.get('admin');
        if (admin) {
            localStorage.setItem('admin', admin); // Save the admin
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://sifas-heart-foundation-1.onrender.com/api/admin/signup', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                if (data.redirect) {
                    ...
                    localStorage.setItem('admin', data.admin); // Store the admin
    ...
    const handleGoogleSignUp = () => {
        ...
        window.open('https://sifas-heart-foundation-1.onrender.com/admin/auth/google', '_self');
    };
    ...
    const handlePasswordModalSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://sifas-heart-foundation-1.onrender.com/api/admin/password-setup', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                localStorage.setItem('admin', data.admin); // Save the admin
